...
for (filename, filepath) in zip(articles_filenames, articles_filepaths):
    with open(filepath, "r") as f:
        ...
        article_html = f'<h1 class="main__title">{article_title}</h1>' + article_html

        filename_html = article_path + ".html"
...
@app.get("/{article_pathname}")
async def article(request: Request, article_pathname):
    with open(f"articles_html/{article_pathname}.html", "r") as f:
        content = f.read()
        return templates.TemplateResponse(request=request, name="article.html", context={ "html_content": content})
